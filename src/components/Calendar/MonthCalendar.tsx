...
const MonthCalendar = ({ entries, onDateClick, className }: MonthCalendarProps) => {
  ...
  const days = useMemo(() => {
    ...
    return daysInMonth.map(day => {
      ...
      const dayEntries = entries.filter(entry => entry.entryDate === dateStr);
      
      // Find dominant mood if multiple entries
      let mood = dayEntries.length > 0 ? dayEntries[0].mood : undefined;
  ...
  }, [entries, currentDate, firstDayOfMonth, lastDayOfMonth]);
